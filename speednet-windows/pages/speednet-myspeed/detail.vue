<template>
  <view class="container">
    ...
    <view class="circle-wrapper">
      <div
        class="sm-sm-loading-css"
        ref="sm_sm_loading"
        :style="smSmStyleObject"
      >
        <text ref="loading_text" :style="smTextStyleObject">{{
          this.progress
        }}</text>
      </div>
...
import { enumConverter } from "../../js_sdk/validator/speednet-game.js";
...
export default {
  ...
  onLoad(e) {
    ...
    console.log("e.autoSpeed:", e.autoSpeed, typeof e.autoSpeed);
    if (e.autoSpeed == "true") {
      this.autoSpeed = true;
    }
    console.log("this.autoSpeed:", this.autoSpeed);
  },
  onBackPress() {
    // 返回键默认返回列表页面
    uni.switchTab({
  ...
  async onReady() {
    ...
    // 如果没有开启，为停止状态
    var curStatus = await this.getRativeStatus();

    if (curStatus == "running") {
      this.setViewStartedStatus();
    } else {
      if (this.autoSpeed == true) {
        await this.startSpeed();
      }
    }
  },
  async onShow() {},
  methods: {
    async getRativeStatus() {
      // 获取真实VPN状态
      await this.$axios
        .get("http://127.0.0.1:9090")
        .then((res) => {
          var status = res.status;
          console.log("corevpn status:", JSON.stringify(status));
          if (status == 200) {
      ...
      console.log(
        " uni.getStorageSync -------------",
        uni.getStorageSync("globalSpeedStatus")
      );
        ...
        try {
          ...
          if (membership == "") {
            ...
            if (membership.is_pause) {
              ...
              var status = await this.getRativeStatus();
              // var status = this.viewStartStatus
              if (status == true) {
                console.log("已经启动corevpn，点击加速无效，直接返回！");
                return;
              }
              ...
              let res = await db
                .collection("speednet-peer")
                .where('_id=="' + this.peerId + '"')
                .get();
              ...
              uni.setStorageSync("lastSpeed", lastSpeed);

              window.electronAPI.startVpn(event);
    ...
    stopAll() {
      window.electronAPI.stopVpn(event);
