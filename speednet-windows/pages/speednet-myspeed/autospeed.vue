...
<script>
	...
	export default {
		...
		methods: {
			async getRativeStatus() { // 获取真实VPN状态
				await this.$axios.get("http://127.0.0.1:9090").then(
					res => {
					...
					error => {
						console.log("9090 catch error..................")
						uni.setStorageSync('globalSpeedStatus', "stoped")
					}
				)
				console.log(" uni.getStorageSync -------------", uni.getStorageSync('globalSpeedStatus'))
					...
					try {
						...
						if (membership == "") {
							...
							if (membership.is_pause) {
								...
								let res = await db.collection("speednet-peer").where('_id=="' + this.peerId + '"')
									.get()
