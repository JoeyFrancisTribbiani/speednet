<template>
  <view class="pages">
    ...
    <uni-list class="uni-list" :border="false" :style="{ height: listHight }">
      <uni-list-item
        class="game-item"
        :to="'/pages/speednet-game/detail?id=' + item.gameId"
        v-for="(item, index) in this.mySpeedList"
        :key="index"
      >
        <!-- 通过header插槽定义列表左侧图片 -->
  ...
  computed: {
    ...
    mySpeedList: {
      get() {
        ...
        console.log(
          "tthis.speedInfo.mySpeedList",
          JSON.stringify(this.speedInfo.mySpeedList)
        );
